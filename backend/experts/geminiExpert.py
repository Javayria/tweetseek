...
class GeminiExpert:
    ...
    def analyze_image(self,b64Image):
        response = self.client.models.generate_content(
            ...
            contents=["Identify the bird from the provided photo. If the bird has multiple words in its name (e.g., 'cedar waxwing'), separate the words using a hyphen ('-'). Only provide the bird's name, not any other details.",types.Part.from_bytes(data=b64Image, mime_type="image/jpg")] #hardcoded to provide gemini with the prompt for identifying the image
        )
